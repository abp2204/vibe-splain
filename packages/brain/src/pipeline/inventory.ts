...
export function inferProductDomain(relPath: string, importSpecs: string[]): ProductDomain {
  ...
  if (
    ...
    p.includes('/bookings/[status]') || p.includes('/booking/[uid]') ||
    p.includes('/bookings/')
  ) return 'booking_management';

  if (
    p.includes('event-types') || p.includes('eventtypes') ||
    p.includes('eventavailabilitytab') || p.includes('eventadvancedtab') ||
    p.includes('eventlimits') || p.includes('eventrecurring')
  ...
  if (
    p.includes('app-store') || p.includes('appstore') ||
    p.includes('/apps/') || p.includes('modules/apps')
  ) return 'apps_marketplace';

  if (
    p.includes('calendar') || p.includes('selected-calendars') ||
    importSpecs.some(s => s.includes('googleapis') || s.includes('@google-cloud/'))
  ) return 'calendar_integrations';

  if (p.includes('video') || p.includes('calvideo') || p.includes('daily.co')) return 'video';
  if (p.includes('onboarding') || p.includes('getting-started')) return 'onboarding';
  if (p.includes('/settings/') || p.includes('/settings.')) return 'settings';
  if (p.includes('/admin/') || p.includes('/admin.')) return 'admin';

  if (
    p.includes('data-table') || p.includes('datatable') ||
    p.includes('datasegment') || p.includes('segment')
  ) return 'data_table';

  if (
    p.includes('shell/navigation') || p.includes('navigationitem') ||
    p.includes('/shell/') || p.includes('sidebar') ||
    p.includes('topnav') || p.includes('mainnav')
  ) return 'shell_navigation';

  if (
    p.includes('form-builder') || p.includes('formbuilder') ||
    p.includes('/forms/') || p.includes('routingforms')
  ) return 'forms';

  if (p.includes('embed')) return 'embed';

  if (
    p.includes('notification') || p.includes('/email/') || p.includes('/emails/') ||
    importSpecs.some(s => s.includes('nodemailer') || s.includes('resend') || s.includes('@sendgrid/'))
